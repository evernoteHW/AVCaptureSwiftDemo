...
//获取 根据字段 codecType 获取 视频 音频 或者 字幕数据
...
{
    return (NSUInteger)_videoCodecCtx ? _videoCodecCtx->height : 0;
}
...
- (kxMovieError) openSubtitleStream: (NSInteger) subtitleStream
{
    ...
    if (codecCtx->subtitle_header_size) {
        ...
        if (s.length) {
            ...
            NSArray *fields = [UIUtils parseEvents:s];
        ...
        if (rect) {
            ...
            if (rect->text) { // rect->type == SUBTITLE_TEXT
                ...
                if (s.length) {
                    ...
                    NSArray *fields = [UIUtils parseDialogue:s numFields:_subtitleASSEvents];
                    if (fields.count && [fields.lastObject length]) {
                        
                        s = [UIUtils removeCommandsFromEventText: fields.lastObject];
                        if (s.length) [ms appendString:s];
                    }                    
                }
            }
