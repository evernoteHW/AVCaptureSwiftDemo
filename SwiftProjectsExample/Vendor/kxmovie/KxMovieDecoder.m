...
#ifdef DEBUG
__unused static void fillSignal(SInt16 *outData,  UInt32 numFrames, UInt32 numChannels)
{
...
__unused static void fillSignalF(float *outData,  UInt32 numFrames, UInt32 numChannels)
{
    static float phase = 0.0;
...
__unused static void testConvertYUV420pToRGB(AVFrame * frame, uint8_t *outbuf, int linesize, int height)
{
...
- (KxVideoFrame *) handleVideoFrame
{
    ...
    if (_videoFrameFormat == KxVideoFrameFormatYUV) {
        ...
        rgbFrame.linesize = _videoFrame->linesize[0];
        rgbFrame.rgb = [NSData dataWithBytes:_videoFrame->data[0]
